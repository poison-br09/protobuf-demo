...
func (c *userServiceClient) GetActivityLog(ctx context.Context, in *GetUserRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[User], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[0], UserService_GetActivityLog_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
func _UserService_GetActivityLog_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(GetUserRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(UserServiceServer).GetActivityLog(m, &grpc.GenericServerStream[GetUserRequest, User]{ServerStream: stream})
}
